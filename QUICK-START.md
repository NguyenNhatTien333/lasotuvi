...
- Đặc tả UI data-rich truyền thống: [spec/spec-2.md](spec/spec-2.md)
- Hệ thống agent và hướng dẫn AI: [.github/agents.md](.github/agents.md)
...
- `spec/spec-2.md`: đặc tả nâng cấp giao diện lá số.
- `.github/instructions/*.instruction.md`: hướng dẫn tác vụ theo module.
- `.github/prompts/*.prompt.md`: prompt mẫu cho bugfix, feature và release gate.

## 7.1. Cập nhật gần đây

- Đã bổ sung mapping trạng thái `saoDacTinh` cho sao Thiên Phủ trong engine.
- Đã thêm regression test cho Thiên Phủ để tránh lệch trạng thái theo vị trí cung.
- Đặc tả UI đã được chuyển từ root sang thư mục `spec/`.
...
Chạy quality gate trước khi commit:
...
Đẩy lên remote `origin` của nhánh hiện tại (`main`):
